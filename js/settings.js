(() => {
  ...
  const STATE_KEY = "playerState";      // { level, xp }
  const WINS_KEY = "gamesPlayed";       // скільки раз грав
  ...
  function xpNeeded(level) { return 100 * Math.pow(2, Math.max(0, level - 1)); }
  ...
  function loadState() {
    try {
      ...
      return { level: Math.max(1, Number(o.level) || 1), xp: Math.max(0, Number(o.xp) || 0) };
    } catch {
      return { level: 1, xp: 0 };
    }
  }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderAvatarGrid() {
      ...
      for (let i = 1; i <= 6; i++) {
        ...
        card.type = "button";
        card.className = "avatar-card" + (locked ? " is-locked" : "") + (i === selectedAvatar ? " is-selected" : "");
        card.setAttribute("aria-label", locked ? `Avatar ${i} locked` : `Avatar ${i}`);
        ...
        card.addEventListener("click", () => {
          if (locked) {
            if (hint) hint.textContent = "🔒 Цей аватар відкриється з вищим рангом.";
            return;
          }
          selectedAvatar = i;
    ...
    saveAvatarBtn?.addEventListener("click", () => {
      ...
      window.renderPlayerInfo?.(); // оновить аватар/ранг на сторінці
    });
    ...
    resetBtn?.addEventListener("click", () => {
      const ok = confirm("Точно скинути ВСЕ? Це видалить ім’я, XP, прогрес глав і аватар.");
      if (!ok) return;
