(() => {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (titleEl) titleEl.textContent = data?.title || `ゲーム ${id}`;
      ...
      <div class="pz-layout">
        <div class="pz-left">
          <div class="pz-board-wrap" id="pzStage">
            <div id="pzBoard" aria-label="パズルボード"></div>
            <img id="pzPreview" class="pz-preview" alt="プレビュー">
            <button class="pz-btn pz-start" id="pzStart" type="button">開始</button>
            <div class="pz-pause-overlay" id="pzPauseOverlay" aria-hidden="true">
              <div class="pz-pause-card">
                <div class="pz-pause-title">一時停止</div>
                <button class="pz-btn" id="pzResume" type="button">再開</button>
                <button class="pz-btn" id="pzExit2" type="button">退出</button>
              </div>
            </div>
          </div>
        </div>
        <div class="pz-right">
          <div class="pz-top">
            ...
            <div class="pz-stats">
              <div>⏳ <span id="pzTime">0</span> 秒</div>
              <div>🏆 <span id="pzBest">—</span></div>
            </div>
          </div>
          <div class="pz-controls">
            <button class="pz-btn" id="pzShuffle" type="button">シャッフル</button>
            <button class="pz-btn" id="pzPause" type="button">一時停止</button>
            <button class="pz-btn" id="pzExit" type="button">退出</button>
          </div>
    ...
    function setCompletePostWinState() {
      ...
      if (hasNext) {
        completeBtn.textContent = "次の章へ →";
        completeBtn.onclick = () => {
          location.href = `./game.html?id=${nextId}`;
        };
      } else {
        completeBtn.textContent = "閉じる";
    ...
    function preparePuzzle() {
      ...
      status.textContent = "「開始」を押してゲームを始めよう。";
    ...
    function checkWin() {
      ...
      status.textContent = bonusXP
        ? `🎉 パズル完成！ +${diffXP} XP • 初回クリア：+${bonusXP} XP`
        : `✅ パズル完成！ +${diffXP} XP`;
